...
const Monodramas = ({ previews }) => (
  <Layout title="OLDstars > Repertoár > Monodramata">
    <div className={styles.previews}>
      {previews
        .filter((item) => item.tag === "monodrama")
        .map((p) => {
          const hasImage = p.images.length > 0;
          const thumbnail = hasImage
            ? p.images[Math.floor(Math.random() * p.images.length)]
            : "/no-image.png";
          return (
            <div className={styles.preview} key={p.title}>
              <Link legacyBehavior href={`/repertoar/${p.slug}`} key={p.slug}>
                <a className="text-bold">
                  <Image
                    src={
                      hasImage
                        ? `/productions/${p.slug}/images/${thumbnail}`
                        : "/no-image.png"
                    }
                    alt={p.title}
                    objectFit="cover"
                  ...
                  <div className="text-center">
                    <div>{p.title}</div>
                    <div className="text-white">{p.writer}</div>
                  </div>
                </a>
              </Link>
            </div>
